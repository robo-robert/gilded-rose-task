...
export abstract class AbstractRecursiveFetcher<T> {
  ...
  protected async repeatedGet({ url, times }: IRepeatedGetProps): Promise<T[]> {
    try {
      return await Promise.all(
        Array.from({ length: times })
          .fill(0)
          .map((_) => this.get({ url })) // eslint-disable-line @typescript-eslint/no-unused-vars
      );
    } catch {
  ...
  abstract recursiveGet({ url, times, iteration, output }: IRecursiveGetProps<T>): Promise<T[]>;
}
